	...
	try {
		...
		if (type == "user") {
			...
			const req = await prisma.$queryRawUnsafe(
				`SELECT payroll, name, user_id_machine, department 
                FROM employee AS e
			...
			const req = await prisma.$queryRawUnsafe(
				`SELECT * FROM DEPT WHERE dept_code LIKE ? AND status = 'Aktif'
                ORDER BY dept_code`,
				`%${val}%`,
			);
			return json(req);
			...
			const req = await prisma.$queryRawUnsafe(
				`SELECT * FROM DEPT WHERE divisi LIKE ? AND status = 'Aktif'
                ORDER BY dept_code`,
				`%${val}%`,
			);
			return json(req);
			...
			const req = await prisma.$transaction(async (tx) => {
                ...
                `,
					start_date,
					end_date,
				)) as { hari_kerja: string; hari_weekend: string }[];
				...
				const newData = Object.fromEntries(Object.entries({ ...Hari_Kerja, ...Day_work, ...getDataLibur }).map(([key, value]) => [key, typeof value == "string" ? value : Number(value)]));

				return { ...newData };
			});
			return json(req);
		} else if (type == "get_cuti_dashboard") {
			// ? Fungsi ini untuk menampilkan detail attendance
			const req = await prisma.$queryRawUnsafe(
				`
			...
			const req = await prisma.$transaction(async (tx) => {
				...
				const newData = Object.fromEntries(Object.entries({ ...attendance }).map(([key, value]) => [key, Number(value)]));
				return { ...newData };
			});
			return json(req);
			...
			const req = await prisma.$transaction(async (tx) => {
				const hasil = (await tx.$queryRawUnsafe(
                    ...
                    ${locals.user.user_type == "HR"? `
                    UNION ALL

                    SELECT s.createdAt AS waktu, 'sppd' AS link, CONCAT("SPPD ", REPLACE(s.sppd_id, "_","/"), " purpose ", s.purpose, " baru dibuat") as deskripsi 
                    FROM SPPD AS s
                    LEFT JOIN SKPD on s.sppd_id = SKPD.sppd_id 
                    WHERE SKPD.sppd_id IS null`
                    : ``}
