...
export const handle: Handle = async ({ event, resolve }) => {
        ...
        if (isLoginPath) {
            ...
            const user = jwt.verify(token, import.meta.env.VITE_TOKEN);
        ...
        // if (token) {
        //     try {
        //         const decoded = jwt.verify(token, import.meta.env.VITE_TOKEN);
        ...
        // if (isLoginPath) {
        //     if (!event.locals.user) {
        //         redirect(303, '/auth/signin'); // Redirect ke login jika tidak ada user
        //     }
        // }
