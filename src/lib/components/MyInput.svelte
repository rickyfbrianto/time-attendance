<script lang="ts">
    ...
    let {type, title = '', name = "", className = "", rows=2, disabled = false,
        ...
        formatDate="yyyy-mm-dd hh:i:s"} = $props()
...
<div class="flex flex-col flex-1 gap-2">
    ...
    <div class={myClass}>
        ...
        {:else if type == 'daterange'}
            <SveltyPicker bind:value={value} isRange inputClasses="w-full rounded-lg border-0 ring-0 ps-3 bg-bgdark text-textdark flex" format={formatDate}/>
        {:else if type == 'datetime'}
            <SveltyPicker bind:value={value} mode={'datetime'} inputClasses="w-full rounded-lg border-0 ring-0 ps-3 bg-bgdark text-textdark" format={formatDate}/>
        {:else}
