...
{#if $appstore.showSidebar}
    <div style="scrollbar-width: none;" transition:fly={{x: "-200px", easing: quadIn}} class="relative flex flex-col bg-bgside px-3 min-w-[16rem] w-[16rem] max-w-[16rem] text-textside overflow-y-scroll font-quicksand font-black">
        ...
        <Modal title={"Pengaturan"} bind:open={formUserState.modal} size={"xl"}>
            ...
            {/if}
            <form transition:fade={{duration:500}} class='flex flex-col gap-4 rounded-lg' enctype="multipart/form-data">
                <div class="flex flex-col p-4 gap-4 border border-slate-300 rounded-lg">
