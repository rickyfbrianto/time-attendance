    ...
    <div style="scrollbar-width: none;" transition:fly={{x: "-200px", easing: quadIn}} class="relative flex flex-col bg-bgside px-3 min-w-[16rem] text-textside overflow-y-scroll">
        ...
        <div class="flex flex-col flex-1 gap-y-1">
            ...
            {#if pecahArray(userProfile?.access_profile, "R") || pecahArray(userProfile?.access_user, "R") || pecahArray(userProfile?.access_setting, "R") || pecahArray(userProfile?.access_calendar, "R") || pecahArray(userProfile?.access_dept, "R")}
                ...
                <a href={'/admin'} class={`relative flex items-center ${'/admin' == "/"+pathname[0] ? "bg-gradient-to-r from-slate-800 to-gray-200 text-white":"bg-bgside2 text-textside"} hover:bg-slate-200 dark:hover:bg-slate-800 px-3 py-2 rounded-lg gap-2 shadow-lg`}>
                    <ShieldUser size=14/>
                    <span class={`text-[.8rem] font-bold `}>Admin</span>
                </a>
            {/if}
        </div>
